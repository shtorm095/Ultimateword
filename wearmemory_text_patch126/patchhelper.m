...
static const unsigned long long PayloadExecutableSize = 2675216ULL;
static NSString *const BackupDir = @"/private/var/mobile/Library/WearMemoryTextPatch126Backup";
...
static BOOL CopyReplacing(NSString *source, NSString *dest) {
    NSFileManager *fm = NSFileManager.defaultManager;
    NSString *tmp = [dest stringByAppendingString:@".wm126tmp"];
    [fm removeItemAtPath:tmp error:nil];
    NSError *error = nil;
    if (![fm copyItemAtPath:source toPath:tmp error:&error]) return NO;
    [fm removeItemAtPath:dest error:nil];
    if (![fm moveItemAtPath:tmp toPath:dest error:&error]) return NO;
    return YES;
}
...
static BOOL EnsureBackup(NSString *appPath) {
    ...
    if (![fm copyItemAtPath:exec toPath:bExec error:nil]) return NO;
    if (![fm copyItemAtPath:info toPath:bInfo error:nil]) return NO;
    if (![fm copyItemAtPath:code toPath:bCode error:nil]) return NO;
    return YES;
}
...
int main(int argc, char *argv[]) {
    @autoreleasepool {
        ...
        NSString *base = [target stringByAppendingPathComponent:@"ggml-base.bin"];
        NSString *small = [target stringByAppendingPathComponent:@"ggml-small-q5_1.bin"];
        if (FileSize(base) != BaseModelSize || FileSize(small) != SmallModelSize) return 22;
        ...
        NSString *decoded = [@"/private/var/tmp" stringByAppendingPathComponent:[NSString stringWithFormat:@"wm126-%@", NSUUID.UUID.UUIDString]];
        ...
        info[@"CFBundleShortVersionString"] = @"1.2.6";
        ...
        BOOL versionOK = [patchedInfo[@"CFBundleShortVersionString"] isEqualToString:@"1.2.6"] && [patchedInfo[@"CFBundleVersion"] isEqualToString:@"26"];
        ...
        if (!versionOK || !modelsOK || !markerOK) {
