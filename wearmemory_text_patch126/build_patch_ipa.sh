...
# Hide the Mach-O magic while the patch executable is embedded in the patcher IPA,
# otherwise TrollStore would re-sign this resource when the patcher itself is installed.
...
xcrun --sdk iphoneos clang -arch arm64 -miphoneos-version-min=15.0 -fobjc-arc -isysroot "$SDK" \
  -framework Foundation -framework UIKit "$ROOT/wearmemory_text_patch126/patcher_main.m" -o "$APP/WearMemoryTextPatch"
...
<key>CFBundleDisplayName</key><string>Text 1.2.6 Patch</string>
...
codesign --force --sign - "$APP/patchhelper"
codesign --force --sign - --entitlements "$WORK/patcher.entitlements" "$APP"
codesign --verify --deep --strict "$APP"

OUT=/tmp/WearMemoryText_1.2.6_InstallPatch_TrollStore_iOS15.ipa
rm -f "$OUT" "$OUT.sha256"
cd "$WORK"
zip -qry "$OUT" Payload
unzip -t "$OUT"
...
# The patch must stay small and must not accidentally contain either model.
! unzip -l "$OUT" | grep -q 'ggml-base.bin'
! unzip -l "$OUT" | grep -q 'ggml-small-q5_1.bin'
SIZE=$(stat -f%z "$OUT")
test "$SIZE" -lt 10000000
printf 'Patch IPA size: %s bytes\n' "$SIZE"
cat "$OUT.sha256"
