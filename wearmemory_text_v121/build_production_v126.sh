...
TEMP=/tmp/wearmemory-text-v126-wrapper.sh
...
source = source.replace("grep -Fx '1.2.1'\", \"grep -Fx '1.2.4'", "grep -Fx '1.2.1'\", \"grep -Fx '1.2.6'")
...
source = source.replace(
    ...
    "new_ipa = 'WearMemoryText_v1.2.6b26_Base_SmallQ5_1_DualPass_Background_9MinuteWatchdog_DeleteCleanAudio_DEElektro_iOS15'",
)
source = source.replace(
    "source = source.replace('WearMemoryText_v1.2.1_Source.zip', 'WearMemoryText_v1.2.4_Source.zip')",
    "source = source.replace('WearMemoryText_v1.2.1_Source.zip', 'WearMemoryText_v1.2.6_Source.zip')",
)
